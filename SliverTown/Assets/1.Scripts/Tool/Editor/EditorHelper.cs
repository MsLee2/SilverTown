...
public class EditorHelper
{
	...
	public static string GetPath(UnityEngine.Object p_clip)
	{
		...
		string[] path_node = retString.Split('/'); //Assets/ResourcesData/Resources/Sound/BGM.wav
	...
	public static void EditorToolTopLayer(BaseData data, ref int selection, ref UnityObject source, int uiWidth) // 탑 레이어
	{
		EditorGUILayout.BeginHorizontal();
        {
			if(GUILayout.Button("ADD", GUILayout.Width(uiWidth)))
            {
				...
				selection = data.GetDataCount() -1; //최종리스트를 선택
				source = null;
            }
